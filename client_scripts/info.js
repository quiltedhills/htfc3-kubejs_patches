...
// the "hold shift" tooltip here is commented out as it's added by pollution filter descriptions
	...
	tooltip.addAdvanced('#forge:deepslate', (item, advanced, text) => {
		if (!tooltip.shift) {
		  ...
		  text.add(1, Text.gold('Obtained by making reinforced variant in an alloy kiln'))
		  text.add(2, Text.gold('A lvl2+ block reinforcer is then needed to remove the reinforcement'))
		}
	})
	...
	tooltip.addAdvanced('myrtrees:latex', (item, advanced, text) => {
		if (!tooltip.shift) {
		  text.add(1, [Text.of('Hold ').gold(), Text.of('Shift ').yellow(), Text.of('to see more info.').gold()])
			...
			text.add(2, Text.gold('If you cannot find a Rubbertree use Rubber Sapling ' ))
			text.add(3, Text.gold('Plant an rubber sapling an dirt, wait or use Tree Fertilizer' ))
			text.add(4, Text.gold('Using a latex bucket, collect latex and wait until solidifies' ))
		}
	})
